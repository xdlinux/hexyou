...
from base.forms import LoginForm,SignupForm,ImageCrop,ImageUpload, AjaxStandard
from base.utils import JsonResponse, upload_image, get_gravatar_url, small_avatar, AjaxForbidden
from base.decorators import json_request, group_admin_required
...
def crop(request,save_to):
    if request.method == "POST" and save_to in settings.IMAGE_SAVE_CHOICES:
        ...
        if form.is_valid():
            ...
            abs_path = os.path.join(settings.MEDIA_ROOT,'tmp',os.path.basename(path))
            ...
            os.remove(abs_path)
            return JsonResponse({'path':os.path.join(settings.MEDIA_URL,save_to,date,filename)})
        else:
            return HttpResponseBadRequest()
    else:
